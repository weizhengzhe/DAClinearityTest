...
public class SerialManager {
    ...
    public boolean open() {
        ...
        try {
            returnValue = this.serialPort.openPort();//Open serial port
...
//            }
            ...
            if (returnValue == true) {
                returnValue = this.serialPort.setParams(
                        SerialPort.BAUDRATE_9600, SerialPort.DATABITS_8,
                        SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
            }

            // set events mask
            if (false == (returnValue = this.serialPort.setEventsMask(
                    SerialPort.MASK_TXEMPTY + SerialPort.MASK_RXCHAR))) {
            ...
            if (returnValue == true) {
                returnValue = this.serialPort.purgePort(
    ...
    public void writeToArduino(String toArduino) {
        ...
        try {
            ...
            while (transmitState == 0) {
                transmitState = this.serialPort.getEventsMask();

                transmitState &= SerialPort.MASK_TXEMPTY;

            }
            writeBytes = this.serialPort.writeBytes(toArduino.getBytes());

        } catch (SerialPortException ex1) {
            Logger.getLogger(SerialManager.class.getName()).log(Level.SEVERE,
                    null, ex1);
            System.exit(0);
        }
    }
